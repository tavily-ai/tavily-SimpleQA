...
class CorrectnessGrade(BaseModel):
    ...
    grade: Annotated[str, "CORRECT, INCORRECT, or NOT_ATTEMPTED (A, B, or C)"]
...
"""
    ...
    async def evaluate(self, inputs: Dict[str, Any], outputs: Dict[str, Any], reference_outputs: Dict[str, Any]) -> dict:
        ...
        grader_prompt = self.OPENAI_GRADER_TEMPLATE.format(
        ...
        # Map full words to letters first
        grade_word_to_letter = {
            "CORRECT": "A",
            "INCORRECT": "B",
            "NOT_ATTEMPTED": "C"
        }

        # Try to match either the full words or the letters
        grade = grade_response.grade.strip()
        if grade in grade_word_to_letter:
            grade = grade_word_to_letter[grade]
        elif grade not in ["A", "B", "C"]:
            grade = "C"  # Default to NOT_ATTEMPTED if we can't parse the response
